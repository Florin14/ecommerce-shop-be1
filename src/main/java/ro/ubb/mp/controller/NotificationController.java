...
public class NotificationController {
    ...
    public ResponseEntity<ResponseWrapperDTO<List<NotificationResponseDTO>>> getUserAllNotifications() {
        if (SecurityContextHolder.getContext().getAuthentication().getPrincipal() instanceof User user) {
            ...
            return ResponseEntity.ok()
                    .body(ResponseWrapperDTO.<List<NotificationResponseDTO>>builder().data(notificationResponseDTOS).build());
        }
        return ResponseEntity.badRequest()
                .body(ResponseWrapperDTO.<List<NotificationResponseDTO>>builder().error("Bad authentication type").build());
    }
}
